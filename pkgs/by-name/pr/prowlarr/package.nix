...
in
  buildDotnetModule {
    ...
    '';
    postInstall = let
      packageInfo = writeText "package_info" ''
        PackageAuthor=Me
        UpdateMethod=External
        UpdateMethodMessage=Foo
        Branch=some-branch
      '';
    in ''
      cp -a -- _output/UI "$out/lib/prowlarr/UI"
      cp ${packageInfo} "$out/package_info"
      cp ${packageInfo} "$out/lib/package_info"
      cp ${packageInfo} "$out/lib/prowlarr/package_info"
    '';
